      ...
      <div class="content" v-if="currentProduct">
        ...
        <div class="item_body">
          <div class="item__left">
            ...
            <span class="new_data" style="font-size:12px;">{{currentProduct.product_type}}</span>
          </div>
          <div class="item__right" style="text-align:right;">
            <span style="flex:1;">{{$t('purchase.productStatus')}}：</span>
            <span class="all_data" style="flex:0 auto;font-size:12px;" v-if="currentProduct.status === 'YYZ'">{{$t('apointRecord.inAppointment')}}</span>
          </div>
        </div>
        <div class="item_body">
          <div class="item__left">
            <span>{{$t('purchase.minShare')}}：</span>
            <span class="new_data" style="font-size:12px;">{{currentProduct.start_money / 10000}}{{$t('apointRecord.tenThousandYuan')}}</span>
          </div>
        </div>
        <div class="item_body">
          <div class="item__left">
            <span>{{$t('purchase.increasingShare')}}：</span>
            <span class="new_data" style="font-size:12px;">1{{$t('apointRecord.tenThousandYuan')}}</span>
          </div>
        </div>
        ...
        <div class="item_foot">
          ...
          <span style="font-size:12px;">{{currentProduct.caopan_time}}</span>
        </div>
        <div class="item_foot" style="display: flex;" v-if="currentProduct.describe">
          <span>{{$t('purchase.productIntroduction')}}：</span>
          <div style="flex: 1;">
            <span style="line-height: 1.5;font-size:12px;">{{currentProduct.describe}}</span>
          </div>
        </div>
      </div>
      <div style="padding: 10px;width: 100%;box-sizing: border-box;padding-bottom:20px;" v-if="currentProduct">
        <div class="form_box">
          <form class="form_area" method="post" @submit.prevent="formSubmit()">
            <div style="padding: 0 10px;">
              <div class="select_type" v-if="pickerArr.length > 1">
                <i class="iconfont icon-unfold"></i>
                <span class="type_title">{{$t('purchase.productName')}}：</span>
              ...
              <div class="item_foot" style='display:flex'>
                ...
                <div style="flex:1;display:flex;flex-wrap:wrap;">
                  <div style="display:flex;width:50%;" v-for="(t, i) in currentPlan.settlement_time" :key="i">
                    <span style="flex:1;line-height: 1.5;font-size:12px;">{{t}}</span>
                  </div>
                </div>
              </div>
              <div class="item_foot" style="display: flex;">
                ...
                <div style="flex: 1;">
                  <span style="line-height: 1.5;font-size:12px;">{{currentPlan.describe}}</span>
                </div>
              </div>
            </div>
